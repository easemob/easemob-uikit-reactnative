...
const callkit_dir = path.resolve(
  root_dir,
  'packages/react-native-chat-callkit'
);
const uikit_dir = path.resolve(root_dir, 'packages/react-native-chat-uikit');
const current_package = JSON.parse(
  fs.readFileSync(path.join(current_dir, 'package.json'), 'utf8')
);
const callkit_package = JSON.parse(
  fs.readFileSync(path.join(callkit_dir, 'package.json'), 'utf8')
);
const uikit_package = JSON.parse(
  fs.readFileSync(path.join(uikit_dir, 'package.json'), 'utf8')
);
