...
// - Add modular_headers for Firebase Swift dependencies
...
if (!fs.existsSync(podfilePath)) {
  ...
  // 3b: Add modular_headers for Firebase deps (required for Swift static libs)
  if (!podfileContent.includes("pod 'GoogleUtilities', :modular_headers")) {
    const modularHeadersPods = [
      "  pod 'GoogleUtilities', :modular_headers => true",
      "  pod 'FirebaseCore', :modular_headers => true",
    ].join('\n');

    const useExpoMatch = podfileContent.match(/^(\s*use_expo_modules!)/m);
    if (useExpoMatch) {
      podfileContent = podfileContent.replace(
        useExpoMatch[0],
        useExpoMatch[0] + '\n\n' + modularHeadersPods
      );
      color_log.success('  Added modular_headers for Firebase dependencies.');
      podfileChanged = true;
    } else {
      color_log.warning('  Could not find use_expo_modules! in Podfile.');
    }
  } else {
    color_log.warning('  Firebase modular_headers already present.');
  }
