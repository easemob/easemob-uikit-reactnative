...
import type { GroupParticipantListItemProps } from './GroupParticipantList.item';

export type useGroupParticipantListApiProps = {};
export function useGroupParticipantListApi(
  props: useGroupParticipantListApiProps
): UseFlatListReturn<GroupParticipantListItemProps> & UseListReturn {
  const {} = props;
  const [data, _setData] = React.useState<
    ReadonlyArray<GroupParticipantListItemProps>
  >([{ id: '1' }]);
  ...
  const ListItem: React.ComponentType<GroupParticipantListItemProps> = () =>
    null;
  ...
  const onMore = () => {};
  const sort: (
    prevProps: GroupParticipantListItemProps,
    nextProps: GroupParticipantListItemProps
  ) => boolean = () => true;
