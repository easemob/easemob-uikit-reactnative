...
import type { SearchNewRequestItemProps } from './SearchNewRequest.item';

export type useSearchNewRequestApiProps = {};
export function useSearchNewRequestApi(
  props: useSearchNewRequestApiProps
): UseFlatListReturn<SearchNewRequestItemProps> & UseListReturn {
  const {} = props;
  const [data, _setData] = React.useState<
    ReadonlyArray<SearchNewRequestItemProps>
  >([{ id: '1' }]);
  ...
  const ListItem: React.ComponentType<SearchNewRequestItemProps> = () => null;
  ...
  const sort: (
    prevProps: SearchNewRequestItemProps,
    nextProps: SearchNewRequestItemProps
  ) => boolean = () => true;
