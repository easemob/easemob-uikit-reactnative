/**
 *	from: https://github.com/tongyy/react-native-draggable
 *
 */
...
export default function Draggable(props: IProps) {
  ...
  const getBounds = React.useCallback(() => {
    const left = offsetFromStart.current.x + (x ?? 0);
    const top = offsetFromStart.current.y + (y ?? 0);
    return {
      left,
      top,
      right: left + (childSize.current.x ?? 0),
      bottom: top + (childSize.current.y ?? 0),
    };
  }, [x, y]);
  ...
  const onPanResponderGrant = React.useCallback(
    (_: GestureResponderEvent, __: PanResponderGestureState) => {
  ...
  const handleOnDrag = React.useCallback(
    (e: GestureResponderEvent, gestureState: PanResponderGestureState) => {
      ...
      const changeX = clamp(
        dx,
        Number.isFinite(minX) ? (minX ?? 0) - left : -far,
        Number.isFinite(maxX) ? (maxX ?? 0) - right : far
      );
      const changeY = clamp(
        dy,
        Number.isFinite(minY) ? (minY ?? 0) - top : -far,
        Number.isFinite(maxY) ? (maxY ?? 0) - bottom : far
      );
