...
For `1.x.x` projects:
Dependency management is relatively crude, problems frequently occur, and you often need to delete `node_modules` and reinstall dependencies.

For `4.x.x` projects:
Installation speed is improved and effective dependency management is added, with friendly prompts for version constraints.

However, for React Native projects, many tools need to use local `node_modules`, so you need to set the `config`.
